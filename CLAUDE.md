...
- Day 26: COMPLETE — `drug_signal_retained` (within-CCLE 5-fold CV, `pctrans/casestudy/braf_vemurafenib.py`) brackets vemurafenib-AUC predictability from BRAF status alone vs. raw HVG expression vs. 64-d embedding on identical CV folds; extended `pctrans-casestudy-analysis` CLI + Figure F6 third panel + `reports/drug_transfer_positioning.json`; real n=41 result: BRAF-alone R²=+0.226/ρ=+0.130 (p=0.42), raw expression R²=−0.041/ρ=−0.167 (p=0.30), embedding R²=−0.333/ρ=−0.187 (p=0.24) — none significant, embedding does not clearly underperform raw expression, so the Day-23 Part-B null reads as underpowered-at-this-n rather than confirmed selective information loss; descriptive (unvalidated) ElasticNet CCLE→patient reference predicts patient AUC 0.780±0.107 inside the CCLE training range; CODE-AE positioning written into `reports/preprint-outline.md` §4.9/§5 (deconfounding-autoencoder + drug-supervised design explains why a lineage-supervised, drug-agnostic embedding isn't expected to transfer)
- Day 27: COMPLETE — `reports/preprint-draft.md` full manuscript draft (Abstract through Back matter, ~4,000 words, evidence Table T2 + reference Table T3, all 7 figures referenced); Figure F2 (`multiseed_baselines_panel.png`: multi-seed kNN@5 distribution + CI alongside baselines/ceiling bar) rendered from existing JSON to close the last figure gap; `reports/blog-03-validation.md` ("I Got 100% Accuracy. Then I Tried to Break It.", 1,328 words) + `linkedin-03.txt` (anchored on the permutation-null figure) + `x-thread-03.txt` (anchored on the 15-lineage confusion heatmap); pre-submission checklist updated (ORCID/venue/citation-check remain, flagged not fabricated); **Phase 2 (Days 15–27) complete**
